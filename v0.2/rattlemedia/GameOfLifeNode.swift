...
class GameOfLifeNode : SKNode {
    ...
    init(life: Life, x: Int, y: Int, radius: CGFloat, buffer: CGFloat = 2, shape: Shape = .Square) {
        self.ref = (x,y)
        self.life = life
        self.radius = radius
        switch shape {
        ...
        case .Square:
            self.shape = SKShapeNode(rectOfSize: CGSize(width: radius * 2, height: radius * 2))
        }
        super.init()
        self.addChild(self.shape)
        self.shape.fillColor = UIColor.yellowColor()
